...
PhysicsList::PhysicsList() {
  ...
  // Optics is still registered always; we can toggle behavior via params
  auto* opt = new G4OpticalPhysics();
  RegisterPhysics(opt);

  // Configure via singleton parameters (G4 11+)
...
// Let all registered physics define their particles first, then ensure GenericIon exists
